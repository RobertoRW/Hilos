 ...
 * 	id = sem_open(key);		# open (must already exists)
 ...
 * for. It doesn't help us if the last process does this (as we have no
 * way of getting control to remove the semaphore) but it will
...
static struct sembuf	op_lock[2] = {
	...
	2, 1, SEM_UNDO	/* then increment [2] to 1 - this locks it */
			/* UNDO to realease the lock if processes exits
...
static struct sembuf	op_endcreate[2] = {
	1, -1, SEM_UNDO,/* decrement [1] (proc counter) with undo on exit */
			/* UNDO to adjust proc counter if processes exits
...
key_t	key;
int	initval;		/* use if we create the semaphore */
{
...
again:
	 ...
	 * 3 members is 0.
	 * Get a lock on the semphore by waiting for [2] to equal 0,
	 * then increment it.
	 * 
	 * There is a race condition here. There is the possibility that
	 * between the semget() above and the semop() below, another
	 * process cal call our sem_close() function which can remove
 ...
 * knows taht the semaphore must already exist.  For example a client
...
int	id;
{
	...
	if (semval > BIGCOUNT)
		...
		if (semop(id,&op_unlock[0],1)<0)		/* unlock */
...
/****************************************************************************
 * General semaphore operation. Increment or decrement by a user_specified
